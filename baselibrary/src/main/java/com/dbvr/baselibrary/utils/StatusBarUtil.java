...
public class StatusBarUtil {
     ...
     */
    @TargetApi(19)
    public static void setTranslucentStatus(Activity activity,boolean isTransparent) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            //5.x开始需要把颜色设置透明，否则导航栏会呈现系统默认的浅灰色
            Window window = activity.getWindow();
            View decorView = window.getDecorView();
            ...
            if (isTransparent){
                ...
                //导航栏颜色也可以正常设置
                window.setNavigationBarColor(activity.getResources().getColor(R.color.color_d5fbfc));
