...
import com.dabangvr.databinding.ActivityVideoPlayBinding;
...
public class VideoActivity extends BaseActivityBinding<ActivityVideoPlayBinding>{
        ...
        adapter = new SlideAdapter<VideoMo>(getContext(),mDatas,R.layout.item_video) {
            ...
            public void convert(Context mContext, BaseRecyclerHolder holder, int position, VideoMo s) {
                ...
                holder.getView(R.id.ivShare).setOnClickListener(view -> {
                    ...
                    intent.putExtra("content",s.getNickName());
                    intent.putExtra("cover",s.getCoverUrl());
                    intent.putExtra("link",s.getVideoUrl());
                    sendBroadcast(intent);
                    adapter.startVideo();
                });

                holder.getView(R.id.ivGoods).setOnClickListener(view -> {
                    getLiveGoods(s.getId());
                });
            }
        };
    ...
    private void getLiveGoods(Object id) {
        Map<String,Object>map = new HashMap<>();
        map.put("videoId",id);
        OkHttp3Utils.getInstance(getContext()).doPostJson(DyUrl.getRoomGoodsList, map, new ObjectCallback<String>(getContext()) {
            ...
            public void onUi(String result) throws JSONException {
                ...
                if (liveGoods!=null && liveGoods.size()>0){
                    BottomDialogUtil2.getInstance(VideoActivity.this).show2(R.layout.recy_no_bg,2, view16 -> {
